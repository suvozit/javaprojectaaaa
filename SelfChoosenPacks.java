...
public class SelfChoosenPacks extends JFrame {
    ...
    private Container c;
    private ImageIcon icon, logo,backgroundImage;
    private JLabel label0, label1, imgLabel,backgroundLabel;
    ...
    SelfChoosenPacks() {
		...
		// Background Image
        backgroundImage = new ImageIcon(getClass().getResource("/images/Background4.jpg")); // Replace with your image path
       ...
       // c.add(backgroundLabel);
        ...
        label1.setBounds(260, 25, 500, 50);
        label1.setFont(f1);
        c.add(label1);
        ...
        label1.setBounds(260, 75, 500, 50);
        label1.setFont(f4);
        c.add(label1);

        String[] cb1Str = { "Choose tour type...", "International", "National" };
        cb1 = new JComboBox(cb1Str);
        cb1.setBounds(420, 84, 250, 30);
        ...
        label1.setBounds(260, 285, 500, 50);
        label1.setFont(f4);
        c.add(label1);

        String[] cb7Str = { "Select vehicle first!" };
        cb7 = new JComboBox(cb7Str);
        cb7.setBounds(420, 296, 250, 30);
        ...
        if ((cb1.getSelectedIndex()) == 0) {
            JOptionPane.showMessageDialog(null, "You forgot to select tour type.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else if ((cb2.getSelectedIndex()) == 0) {
            JOptionPane.showMessageDialog(null, "You forgot to select destination.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else if ((cb3.getSelectedIndex()) == 0) {
            JOptionPane.showMessageDialog(null, "You forgot to select total person number.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else if ((cb4.getSelectedIndex()) == 0) {
            JOptionPane.showMessageDialog(null, "You forgot to select total number of days.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else if ((cb5.getSelectedIndex()) == 0) {
            JOptionPane.showMessageDialog(null, "You forgot to select hotel type.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else if (((cb1.getSelectedIndex()) == 2) && ((cb6.getSelectedIndex()) == 0)) {
            JOptionPane.showMessageDialog(null, "You forgot to select travel by.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else if (((cb1.getSelectedIndex()) == 2) && ((cb6.getSelectedIndex()) != 3) && ((cb7.getSelectedIndex()) == 0)) {
            JOptionPane.showMessageDialog(null, "You forgot to select vehicle type.", "Warning!", JOptionPane.WARNING_MESSAGE);
        } else {
            // Calculate the total cost
            int totalCost = 0;
            if (cb1.getSelectedIndex() == 1) {
                // International tour costs
                totalCost = calculateInternationalCost();
            } else if (cb1.getSelectedIndex() == 2) {
                // National tour costs
                totalCost = calculateNationalCost();
            }

            // Show the total cost to the user
            JOptionPane.showMessageDialog(null, "Total Cost: " + totalCost + " tk", "Total Cost", JOptionPane.INFORMATION_MESSAGE);

            // Ask for confirmation
            int response = JOptionPane.showConfirmDialog(null, "Do you want to proceed?", "Confirmation", JOptionPane.YES_NO_OPTION);

            // If user selects "Yes", proceed to the payment page
            if (response == JOptionPane.YES_OPTION) {
                setVisible(false);
                Payment frame = new Payment(); // Assuming Payment is another JFrame for payment page
                frame.setVisible(true);
            }
        }
    }

    // Method to calculate international cost
    private int calculateInternationalCost() {
        int cb1Cost = 100;
        int cb2Cost = 0;
    ...
    // Method to calculate national cost
    private int calculateNationalCost() {
        int cb1Cost = 10000;
        int cb2Cost = 0;
